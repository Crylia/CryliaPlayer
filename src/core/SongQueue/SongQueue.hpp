...
#include "ConditionalCircularLinkedList/ConditionalCircularLinkedList.hpp"
...
private:
  ...
  // The name is confusing, but its because the queue itself has priority over
  // the normal one
...
public:
  ...
  /**
   * @brief Will put the given song from the priority or normal queue as the top
   * song and remove it from the list
   *
   * @param song
   */
  ...
  void MoveSongInPriorityQueue(Song *songToMove, Song *otherSong,
