...
void Audio::StartMusic( ) {
  if (Mix_PlayingMusic( ) == 0) {
    ...
    Mix_PlayMusic(music, (int)this->loop);
  }
}
...
void Audio::PlaySong(const QString path) {
  if (music)
    Mix_FreeMusic(music);

  this->path = path;

  music = Mix_LoadMUS(path.toStdString( ).c_str( ));
  if (!music) {
...
QString Audio::GetTitle( ) {
  std::cout << Mix_GetMusicTitleTag(music) << std::endl;
  return QString::fromStdString(Mix_GetMusicTitleTag(music));
}

QString Audio::GetArtist( ) {
  return QString::fromStdString(Mix_GetMusicArtistTag(music));
}
...
QPixmap Audio::GetAlbumCover( ) {
  ...
  if (avformat_open_input(&fc, this->path.toStdString( ).c_str( ), NULL, NULL) != 0) {
