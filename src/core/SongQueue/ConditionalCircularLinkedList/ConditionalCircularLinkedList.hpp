...
public:
  ConditionalCircularLinkedList()
      : head(nullptr), isLinked(false), current(nullptr) {}
  ConditionalCircularLinkedList(bool isLinked)
      : head(nullptr), isLinked(isLinked), current(nullptr) {}
   ...
   * If its not supposed to loop then we break the link and it will be a normal
   * list.
  ...
  /**
   * @brief Advance the circular list by one, this means setting the "current"
